...
(function (jQuery) { //防止jQuery污染
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
					...
					var block = jQuery("<div>", {
                        ...
                        "title": series.name + "[plan], " + DateUtils.daysBetween(series.plan.start, series.plan.end) + " days",
                        "css": {
                            "width": (( (DateUtils.daysBetween(series.plan.start, series.plan.end) + 1) * cellWidth) - 9) + "px",
                            "margin-left": (((DateUtils.daysBetween(start, series.plan.start)) * cellWidth) + 3) + "px",
                            "height": 13 + "px"
                        }
                    });

                    var block2 = jQuery("<div>", {
                        "class": "ganttview-block-true",
                        "title": series.name + "[true], " + DateUtils.daysBetween(series.true.start, series.true.end) + " days",
                        "css": {
                            "width": (( (DateUtils.daysBetween(series.true.start, series.true.end) + 1) * cellWidth) - 9) + "px",
                            "margin-left": (((DateUtils.daysBetween(start, series.true.start) * cellWidth)) + 3) + "px",
                            "height": 13 + "px"
                        }
                    });
                    ...
                    addBlockData(block, data[i], series.plan); 
                    addBlockData(block, data[i], series.true); 
                    // if (series.color) { 先写在genttView.css中 
                        // block.css("background-color", series.color);
                    // }
                    block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(DateUtils.daysBetween(series.plan.start, series.plan.end) + 1));
                    block2.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(DateUtils.daysBetween(series.true.start, series.true.end) + 1));

                    jQuery(rows[rowIdx]).append(block);
                    jQuery(rows[rowIdx]).append(block2);
                    rowIdx = rowIdx + 1;
                }
            }
        }
    ...
    var DateUtils = {
		...
		getBoundaryDatesFromData: function (data, minDays) {
			...
			for (var i = 0; i < data.length; i++) {
				for (var j = 0; j < data[i].series.length; j++) {
					var start = Date.parse(data[i].series[j].plan.start);
					var end = Date.parse(data[i].series[j].plan.end)
